...
public class PhysicsPlayerControl : MonoBehaviour {
    ...
    public float jumpThrust = 50000.0f;
    void StartJump() {
        body.AddForce(transform.up * jumpThrust);
        Debug.Log("started jump", this);
    }

    public AnimationCurve movementForceAtSpeed;
    public float moveThrust = 5000.0f;
    void StartMove(float move_input, float speedLimiterProgress) {
        speedLimiterProgress = Mathf.Clamp(speedLimiterProgress, 0,1);

        float targetForce = moveThrust * Math.Sign(move_input);
        targetForce *= movementForceAtSpeed.Evaluate(speedLimiterProgress);

        body.AddForce(transform.right * targetForce);
        /*~ 
